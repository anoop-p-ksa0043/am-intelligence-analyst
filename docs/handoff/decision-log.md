...
- 2026-03-23: **Deployed to Vercel** via GitHub integration. Repository: `anoop-p-ksa0043/am-intelligence-analyst`. Added `postinstall` script to `package.json` for Prisma client generation on build. Environment variables set: DATABASE_URL, NEXTAUTH_URL, NEXTAUTH_SECRET, GEMINI_API_KEY, ENABLE_REVAMP_UI.
- 2026-03-23: **Enforced root route authentication** in `app/page.tsx`. For revamp UI, checks session and redirects to `/login` if not authenticated, else to `/accounts`. Rationale: Ensures users are authenticated before accessing the app, improving security and user experience.
